...
# golden-tested. Usage: img2sprites.py <in.png> <name> <size 8|16> [out.h] [--mode2] [--zx0]
# --zx0 emits the pattern table ZX0-packed (unpack on-target with UnZX0); the small
# colour array stays raw. Compression is host-side (see zx0pack.py); the MSX only unpacks.
import sys
import zx0pack
...
def main():
    ...
    if len(argv) < 3:
        sys.exit("usage: img2sprites.py <in.png> <name> <size 8|16> [out.h] [--mode2] [--zx0]")
    ...
    pat_c, stored = zx0pack.emit(name, "pattern", r["pattern"], zx0)   # bulk -> packable
    lines = [
        f"// {out} -- GENERATED by img2sprites.py from {path} (do not edit).",
        f"// MSX {r['size']}x{r['size']} sprite bank: {r['count']} sprites{mnote} "
        f"({len(r['pattern'])} pattern bytes{', ZX0-packed' if zx0 else ''}).",
        '#include "types.h"', "",
        pat_c,
        carr(name + "_colour", r["colour"]),   # small -> raw
        f"#define {U}_COUNT {r['count']}",
        f"#define {U}_SIZE {r['size']}",
        f"#define {U}_PLANES {r['planes']}",
    ]
    open(out, "w").write("\n".join(lines) + "\n")
    packnote = f" -> {stored}B ZX0 ({100 - 100*stored//len(r['pattern'])}% saved)" if zx0 else ""
    print(f"wrote {out}: {r['count']} {r['size']}x{r['size']} sprites"
          f"{' x ' + str(r['planes']) + ' planes' if mode2 else ''}, "
          f"{len(r['pattern'])}B pattern{packnote} + {len(r['colour'])}B colour")
