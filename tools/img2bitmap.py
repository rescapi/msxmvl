...
# Usage: img2bitmap.py <in.png> <name> <screen 5|7|8|12> [out.h] [--optimal] [--zx0]
# --zx0 emits the bitmap ZX0-packed (unpack on-target with UnZX0/UnZX0_VRAM); the small
# palette stays raw. Compression is host-side (see zx0pack.py); the MSX only unpacks.
import sys
import zx0pack
...
def main():
    ...
    if len(argv) < 3:
        sys.exit("usage: img2bitmap.py <in.png> <name> <screen 5|7|8|12> [out.h] [--optimal] [--zx0]")
    ...
    bitmap_c, stored = zx0pack.emit(name, "bitmap", r["data"], zx0)   # bulk array -> packable
    lines = [
        f"// {out} -- GENERATED by img2bitmap.py from {path} (do not edit).",
        f"// MSX SCREEN {r['screen']} {kind}: {r['w']}x{r['h']}, {r['bpp']} bpp, "
        f"{len(r['data'])} VRAM bytes{' (ZX0-packed)' if zx0 else ''}.",
        '#include "types.h"', "",
        bitmap_c,
    ]
    if r["pal"] is not None:
        lines.append(carr(name + "_palette", r["pal"]))  # 16 colours, VDP format (RB,G) -- raw
    lines += [
        f"#define {U}_W {r['w']}",
        f"#define {U}_H {r['h']}",
    ...
    packnote = f" -> {stored}B ZX0 ({100 - 100*stored//len(r['data'])}% saved)" if zx0 else ""
    print(f"wrote {out}: SCREEN {r['screen']} {r['w']}x{r['h']} {r['bpp']}bpp, "
          f"{len(r['data'])}B bitmap{packnote}{extra}{palkind}, round-trip OK")
