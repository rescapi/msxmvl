#!/usr/bin/env python3
# Generate unzx7_corpus.h: a set of ZX7 round-trip test vectors packed by the
# REFERENCE encoder (Einar Saukas's `zx7`, default forward mode -- the format our
# UnZX7 decodes). This makes the encoder the ground truth: the example
...
# this. It builds the encoder from the vendored ZX7 reference source if no `zx7`
# is on PATH (same code as github.com/einar-saukas/ZX7).
import os, subprocess, sys, tempfile, shutil, random

HERE = os.path.dirname(os.path.abspath(__file__))
Z88_SRC = "/home/remco/tools/z88dk/src/zx7"

def find_encoder():
    exe = shutil.which("zx7")
    if exe:
        return exe
    # build from vendored reference source
    out = os.path.join(tempfile.gettempdir(), "zx7ref_gen")
    srcs = [os.path.join(Z88_SRC, f) for f in
            ("zx7.c", "compress.c", "optimize.c")]
    if all(os.path.exists(s) for s in srcs):
        subprocess.run(["gcc", "-O2", "-I", Z88_SRC, "-o", out, *srcs], check=True)
        return out
    sys.exit("no `zx7` encoder on PATH and no vendored source at " + Z88_SRC)

def pack(enc, raw):
    with tempfile.TemporaryDirectory() as d:
        i = os.path.join(d, "i.bin"); o = os.path.join(d, "o.zx7")
        open(i, "wb").write(raw)
...
def main():
    ...
    lines = [
        "// unzx7_corpus.h -- GENERATED by unzx7_corpus_gen.py (do not edit).",
        "// ZX7 round-trip vectors packed by the reference encoder; the oracle for",
        "// unzx7_01_roundtrip.c. Each entry: packed stream + original + length.",
        "#ifndef MVCZ80_UNZX7_CORPUS_H",
        "#define MVCZ80_UNZX7_CORPUS_H",
        "#include \"types.h\"",
        "",
    ]
    entries = []
    ...
    for name, raw in vectors:
        ...
        lines.append(carr(f"g_zx7_pk_{name}", packed))
        lines.append(carr(f"g_zx7_raw_{name}", raw))
        entries.append((name, len(raw)))
    lines.append(f"#define ZX7_CORPUS_MAXRAW {maxraw}")
    lines.append(f"#define ZX7_CORPUS_COUNT  {len(entries)}")
    lines.append("typedef struct { const u8* packed; const u8* raw; u16 len; } ZX7Vec;")
    lines.append("static const ZX7Vec g_zx7_corpus[ZX7_CORPUS_COUNT] = {")
    for name, ln in entries:
        lines.append(f"\t{{ g_zx7_pk_{name}, g_zx7_raw_{name}, {ln} }},")
    lines.append("};")
    lines.append("#endif")
    open(os.path.join(HERE, "unzx7_corpus.h"), "w").write("\n".join(lines) + "\n")
    print("wrote unzx7_corpus.h:", ", ".join(f"{n}={l}B" for n, l in entries))
