...
Because the instruments are built in, a note is just three register writes — instrument + volume,
pitch, then key-ON:
...
After `MSXMusic_Initialize()` finds the chip (`R[] = a5 02 02` — an **external** FM-PAC in slot 2),
`play_piano_note()` sounds an A on channel 0, and the emulated OPLL's own register `0x30` reads back
`0x50`, so the write really reached the chip. *(tested: `msxmusic_01_note.c`, on an FM-PAC)*
