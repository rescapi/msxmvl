// msxmvl clean-room reimplementation of MSXgl "memory_mapper" module.
// Public names + signatures match MSXgl exactly (drop-in replacement).
// Derived from MSXgl engine/src/memory_mapper.h (interface only) +
// standard MSX memory-mapper knowledge (ports 0xFC..0xFF, ASCII/MSX-DOS2 style).
//
// In MSXgl these are `inline` functions living in memory_mapper.h. msxmvl
// exposes them as REAL, linkable functions with the exact same names/signatures
// so a single difftest driver can link against real MSXgl OR msxmvl.
//
// The MSX memory mapper switches 16 KB segments into the four Z80 pages by
// writing the segment number to one I/O port per page:
//   0xFC -> page 0 (0000-3FFF), 0xFD -> page 1 (4000-7FFF),
//   0xFE -> page 2 (8000-BFFF), 0xFF -> page 3 (C000-FFFF).
#ifndef MSXMVL_MEMORY_MAPPER_H
#define MSXMVL_MEMORY_MAPPER_H

#include "types.h"

//=============================================================================
// SDCC attribute macros (mirror MSXgl core.h)
//=============================================================================
#ifndef __PRESERVES
#define __PRESERVES		__preserves_regs
#endif

//=============================================================================
// DEFINES
//=============================================================================

#define P_MEM_P0					0xFC // (252) selects the segment in page 0 (0000-3FFFh).
#define P_MEM_P1					0xFD // (253) selects the segment in page 1 (4000-7FFFh).
#define P_MEM_P2					0xFE // (254) selects the segment in page 2 (8000-BFFFh).
#define P_MEM_P3					0xFF // (255) selects the segment in page 3 (C000-FFFEh).

#define MEM_SETSEGMENT(page, seg)	MemMap_SetPage##page(seg)

__sfr __at(P_MEM_P0)				g_PortMemPage0; // Port to access memory mapper for memory page 0 (0000-3FFFh)
__sfr __at(P_MEM_P1)				g_PortMemPage1; // Port to access memory mapper for memory page 1 (4000-7FFFh)
__sfr __at(P_MEM_P2)				g_PortMemPage2; // Port to access memory mapper for memory page 2 (8000-BFFFh)
__sfr __at(P_MEM_P3)				g_PortMemPage3; // Port to access memory mapper for memory page 3 (C000-FFFEh)

//=============================================================================
// FUNCTIONS
//=============================================================================
// The header declares no __PRESERVES() contract on any of these (they are
// `inline` in MSXgl), so they follow the standard SDCC calling convention.

// Function: MemMap_SetPage0
// Set memory mapper segment to switch in page 0 (0000-3FFFh).
void MemMap_SetPage0(u8 seg);

// Function: MemMap_SetPage1
// Set memory mapper segment to switch in page 1 (4000-7FFFh).
void MemMap_SetPage1(u8 seg);

// Function: MemMap_SetPage2
// Set memory mapper segment to switch in page 2 (8000-BFFFh).
void MemMap_SetPage2(u8 seg);

// Function: MemMap_SetPage3
// Set memory mapper segment to switch in page 3 (C000-FFFEh).
void MemMap_SetPage3(u8 seg);

// Function: MemMap_Initialize
// Initialize memory mapper to the standard MSX boot layout:
// page3=seg0, page2=seg1, page1=seg2, page0=seg3.
void MemMap_Initialize(void);

#endif // MSXMVL_MEMORY_MAPPER_H
